<script lang="ts">
	...
	function refreshFilterOptions() { programOptions = uniqueListValues(loadedRows); employerOptions = uniqueSorted(loadedRows.map((row) => label(row.employer))); sourceOptions = uniqueSorted(loadedRows.map((row) => label(row.referralSource))); reasonOptions = uniqueSorted(loadedRows.map((row) => label(row.referralReason))); selectedPrograms = selectedPrograms.filter((v) => programOptions.includes(v)); selectedEmployers = selectedEmployers.filter((v) => employerOptions.includes(v)); selectedSources = selectedSources.filter((v) => sourceOptions.includes(v)); selectedReasons = selectedReasons.filter((v) => reasonOptions.includes(v)); }
	function passesFilters(row: Row) { const programsOk = selectedPrograms.length === 0 || selectedPrograms.some((value) => (row.programs ?? []).includes(value)); const employersOk = selectedEmployers.length === 0 || selectedEmployers.includes(label(row.employer)); const sourcesOk = selectedSources.length === 0 || selectedSources.includes(label(row.referralSource)); const reasonsOk = selectedReasons.length === 0 || selectedReasons.includes(label(row.referralReason)); return programsOk && employersOk && sourcesOk && reasonsOk; }
	...
	$: {
		...
		const range = currentRange(); const comparison = resolvedComparisonRange(); currentRangeLabel = range ? `${range.startDate} to ${range.endDate}` : ''; comparisonRangeLabel = comparisonEnabled && comparison ? `${comparison.startDate} to ${comparison.endDate}` : ''; filteredRows = range ? loadedRows.filter((row) => passesFilters(row) && inRange(row.referralAt, range.startDate, range.endDate)) : []; comparisonRows = comparison ? comparisonLoadedRows.filter((row) => passesFilters(row) && inRange(row.referralAt, comparison.startDate, comparison.endDate)) : []; chips = loadedRows.length > 0 ? activeFilterChips() : []; modalFilterLabels = chips.map((chip) => chip.label); table = filteredRows.map((row) => ({ member: row.memberName ?? row.memberEmail ?? row.memberId, employer: label(row.employer), programs: row.programs?.join(', ') || '-', referralSource: label(row.referralSource), referralReason: label(row.referralReason), referralDate: row.referralDate ?? '-' })); sourceTrendData = range ? buildEventDateSeries(filteredRows, (row) => row.referralAt, (row) => [label(row.referralSource)], range) : { dates: [], series: [], xAxisLabel: 'Date' }; reasonTrendData = range ? buildEventDateSeries(filteredRows, (row) => row.referralAt, (row) => [label(row.referralReason)], range) : { dates: [], series: [], xAxisLabel: 'Date' }; comparisonSourceTrendData = comparison ? buildEventDateSeries(comparisonRows, (row) => row.referralAt, (row) => [label(row.referralSource)], comparison) : { dates: [], series: [], xAxisLabel: 'Date' }; comparisonReasonTrendData = comparison ? buildEventDateSeries(comparisonRows, (row) => row.referralAt, (row) => [label(row.referralReason)], comparison) : { dates: [], series: [], xAxisLabel: 'Date' };
	}
