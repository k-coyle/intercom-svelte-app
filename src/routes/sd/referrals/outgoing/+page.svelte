<script lang="ts">
	...
	function destinationLabel(row: Row): string { return normalizeFilterValue(row.outgoingReferral, { blankLabel: SD_BLANK_FILTER_LABELS.outgoingReferralDestination }); }
	function reasonLabel(row: Row): string { return normalizeFilterValue(row.outgoingReferralReason, { blankLabel: SD_BLANK_FILTER_LABELS.referralReason }); }
	function coachLabel(row: Row): string { return normalizeFilterValue(row.coachName, { blankLabel: SD_BLANK_FILTER_LABELS.coach }); }
	...
	function refreshFilterOptions() { programOptions = mergeFilterOptions(SD_FILTER_DEFAULT_OPTIONS.programs, loadedRows.flatMap((row) => valuesOrBlank(row.programs)), { blankLabel: SD_BLANK_FILTER_LABELS.program }); employerOptions = mergeFilterOptions(SD_FILTER_DEFAULT_OPTIONS.employers, loadedRows.map((row) => row.employer), { blankLabel: SD_BLANK_FILTER_LABELS.employer }); destinationOptions = mergeFilterOptions(SD_FILTER_DEFAULT_OPTIONS.outgoingReferralDestinations, loadedRows.map((row) => row.outgoingReferral), { blankLabel: SD_BLANK_FILTER_LABELS.outgoingReferralDestination }); reasonOptions = mergeFilterOptions(SD_FILTER_DEFAULT_OPTIONS.referralReasons, loadedRows.map((row) => row.outgoingReferralReason), { blankLabel: SD_BLANK_FILTER_LABELS.referralReason }); coachOptions = mergeFilterOptions(SD_FILTER_DEFAULT_OPTIONS.coaches, loadedRows.map((row) => row.coachName), { blankLabel: SD_BLANK_FILTER_LABELS.coach }); selectedPrograms = retainSelectedFilterValues(selectedPrograms, programOptions); selectedEmployers = retainSelectedFilterValues(selectedEmployers, employerOptions); selectedDestinations = retainSelectedFilterValues(selectedDestinations, destinationOptions); selectedReasons = retainSelectedFilterValues(selectedReasons, reasonOptions); selectedCoaches = retainSelectedFilterValues(selectedCoaches, coachOptions); }
	function passesFilters(row: Row) { const programsOk = matchesSelectedListFilter(selectedPrograms, row.programs, { blankLabel: SD_BLANK_FILTER_LABELS.program }); const employersOk = matchesSelectedFilter(selectedEmployers, row.employer, { blankLabel: SD_BLANK_FILTER_LABELS.employer }); const destinationsOk = matchesSelectedFilter(selectedDestinations, row.outgoingReferral, { blankLabel: SD_BLANK_FILTER_LABELS.outgoingReferralDestination }); const reasonsOk = matchesSelectedFilter(selectedReasons, row.outgoingReferralReason, { blankLabel: SD_BLANK_FILTER_LABELS.referralReason }); const coachesOk = matchesSelectedFilter(selectedCoaches, row.coachName, { blankLabel: SD_BLANK_FILTER_LABELS.coach }); return programsOk && employersOk && destinationsOk && reasonsOk && coachesOk; }
	...
	$: {
		...
		const range = currentRange(); const comparison = resolvedComparisonRange(); currentRangeLabel = range ? `${range.startDate} to ${range.endDate}` : ''; comparisonRangeLabel = comparisonEnabled && comparison ? `${comparison.startDate} to ${comparison.endDate}` : ''; filteredRows = range ? loadedRows.filter((row) => passesFilters(row) && inRange(row.createdAt, range.startDate, range.endDate)) : []; comparisonRows = comparison ? comparisonLoadedRows.filter((row) => passesFilters(row) && inRange(row.createdAt, comparison.startDate, comparison.endDate)) : []; chips = loadedRows.length > 0 ? activeFilterChips() : []; modalFilterLabels = chips.map((chip) => chip.label); table = filteredRows.map((row) => ({ member: row.memberName ?? row.memberEmail ?? row.memberId ?? row.conversationId, employer: employerLabel(row), programs: programLabels(row).join(', '), destination: destinationLabel(row), reason: reasonLabel(row), coach: coachLabel(row), createdDate: row.createdDate ?? '-' })); destinationTrendData = range ? buildEventDateSeries(filteredRows, (row) => row.createdAt, (row) => [destinationLabel(row)], range) : { dates: [], series: [], xAxisLabel: 'Date' }; reasonTrendData = range ? buildEventDateSeries(filteredRows, (row) => row.createdAt, (row) => [reasonLabel(row)], range) : { dates: [], series: [], xAxisLabel: 'Date' }; comparisonDestinationTrendData = comparison ? buildEventDateSeries(comparisonRows, (row) => row.createdAt, (row) => [destinationLabel(row)], comparison) : { dates: [], series: [], xAxisLabel: 'Date' }; comparisonReasonTrendData = comparison ? buildEventDateSeries(comparisonRows, (row) => row.createdAt, (row) => [reasonLabel(row)], comparison) : { dates: [], series: [], xAxisLabel: 'Date' };
	}
